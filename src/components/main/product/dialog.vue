<template>
  <el-dialog
    ...
    ><div class="click_wrapper">
      <div class="click_product_photo_wrapper"  ><div class='click_product_photo' :style="{ backgroundImage: 'url(' + dataNews.imgdata + ')' }"></div></div>
      <div class="click_product_content">
        <div class="product_name">{{dataNews.productname}}
          </div>
          <div class="product_introduction">{{dataNews.introduction}}</div>
           <div class="order">
             <div class="float">
             <span style="padding-top: 4px;
    display: inline-block;">规格：</span> 
             </div>
             <el-tag class="float" v-if="dataNews.optionid=='1'" type="info" >默认规格</el-tag>
             <el-radio-group class="float" v-else v-model="radio1" size="small" fill='#fdc224' >
               <template v-for="(item,index) in dataNews.product_option" :key="index">
...
export default defineComponent({
  ...
  setup(props: any, context) {
    ...
    return { dialogShow, cancel, dataNews,radio1 };
  },
});
</script>
<style>
